...
"SELECT projects.title, SUM(projects.funding_goal) FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY pledges.amount HAVING SUM(pledges.amount) = projects.funding_goal"
end
