...
"SELECT projects.title, SUM(pledges.amount) FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY pledges.amount HAVING SUM(pledges.amount) > projects.funding_goal"
end
